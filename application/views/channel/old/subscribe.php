 <div class="container-fluid pad_adjust  mar-top-30">
  ...
  <h4><a href="javascript:;">My Property</a>
    <i class="fa fa-angle-right"></i>Subcribe</h4>
  </div>  
  <hr>
...
<div class="row" style="height: 600px; overflow-y: scroll;">
<?php 
$subscribe_plans = $this->db->query("SELECT * FROM subscribe_plan where status=1 order by Grouptype")->result_array();
if(count($subscribe_plans)!=0)
{
    $grouptypes="";
  foreach($subscribe_plans as $plan)
  {
    extract($plan);
    $free_plan_exist = get_data(MEMBERSHIP,array('user_id'=>current_user_type(),'buy_plan_type' => 'Free'))->num_rows();
  ...
  <?php if($plan_types=='Free') {
    ...
    { 
      echo '<div> <h4 style="color:#7cc623";>'.$Grouptype.'</4> </div>';
...
<div class="radio">
  <label>
    <input type="radio" name="plan" id="optionsRadios1" value="<?php echo $plan_id;?>" class="plan_details">
  <?php echo '<hr style="color: #7cc623;" />';} ?>
    <?php if($plan_types=='Free') {
  ...
  ?>
    ...
    <?php  echo '<hr style="color: #0056b2;" />';} }else{?>
      </label>
      </div>
      </div>
    <?php } ?>    
...
<div class="row" id="enable_button_subscribe">
<div class="col-md-12 col-md-12">
