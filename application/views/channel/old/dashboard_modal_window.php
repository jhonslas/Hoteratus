...
?>
 <div class="portlet-title">
          <div class="caption col-md-11">
              ...
              <span class="caption-subject font-green bold uppercase">Today's Cancellations</span>
          </div>
...
if($room_type)
{
	...
	{  
		$status=$room->status;
		if($status==11 || $status == "Book" || $status == "BOOKED" ||  $status == "Commit" || $status == "Confirmed" || $status == "new"  || $status == "AmendBooking"  || $status == "ConfirmBooking" ) $status_type='<button type="button" class="btn btn-info btn-sm hidden-print">New booking</button> <span class="display_none view-print">New booking</span>';
		...
		else if($status==13 || $status == "Cancel" || $status == "CANCELLED" || $status == "Cancel" || $status=="Cancelled" || $status=="CancelBooking") $status_type='<button type="button" class="btn btn-danger btn-sm hidden-print">Cancellation</button> <span class="display_none view-print">Cancellation</span>';	
		if($room->channel_id==2)
		{
			if($room->guest_name!='')
			{
...
elseif($room->channel_id==19)
{
    $roomName = @$this->db->query("select c.property_name from import_mapping_AGODA as a left join ".MAP." as b  on a.map_id = b.import_mapping_id  and b.channel_id=19 left join ".TBL_PROPERTY." as c on b.property_id=c.property_id where roomtype_id =".$room->roomtypeId." and rate_type_id=".$room->rateplanid)->row()->property_name;
}
if($roomName)
{
	echo ucfirst($roomName);
