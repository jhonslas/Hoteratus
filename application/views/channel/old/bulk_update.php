<div id="dateTimemodal" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title">Select Date Time</h4>
      </div>
      <div class="modal-body">
		<label>Select Date Time</label>
        <p><input type="text" id="showResults" class="form-control" style="width:100%" placeholder="Select Date Time"/></p>
      </div>
      <div class="modal-footer">
		<button type="button" class="btn btn-info" data-dismiss="modal" onclick="if($('#showResults').val() != ''){window.open('getResults/'+encodeURIComponent($('#showResults').val()),'_blank');}else{ alert('Please Select Date Time');return false;}">View</button>
...
<div class="container-fluid pad_adjust  mar-top-30">
	<a type="button" target="_blank" class="btn btn-sm btn-success" style="float: right;" href="#" data-toggle="modal" data-target="#dateTimemodal">Show Results</a>
	<form id="bulk_update_form1" class="" action="<?php echo lang_url();?>inventory/bulk_update/update" method="post">
    ...
    <div class="col-md-3 col-sm-5 col-xs-12 cls_resp50">
        ...
        <ul class="list-unstyled cls_bluk_list mar-no pad-no">
			<li class="col-md-7 col-sm-6 pad-no">
				...
				<label for="checkbox16"> All (Main Calendar) </label>
				</div>
			</li>
		<?php
		if(count($con_cha)!=0) 
		...
		<div id="additional_dates">
		</div>
		<div class="clearfix"> </div>
		<div class="clearfix" id="addnewDates"><input value=" + Add more" class="form-control btn btn-md btn-default" type="button" onclick="addDates();"> </div>
...
function addDates(){
	...
	$('#additional_dates').append('<div class="clearfix"> </div><div class="form-group"><label class="col-sm-4 control-label">Start date </label><div class="col-sm-8"><input type="text" class="form-control widh" id="dp'+counter+'" required name="start_date'+counter+'"></div></div><div class="clearfix"> </div><div class="form-group"><label class="col-sm-4 control-label">End date </label><div class="col-sm-8"><input type="text" class="form-control widh" id="dp'+counter+'-p" required name="end_date'+counter+'"></div></div>');
	var nowTemp = new Date();
	var now = new Date(nowTemp.getFullYear(), nowTemp.getMonth(), nowTemp.getDate(), 0, 0, 0, 0);

	var checkin = $('#dp'+counter+'').datepicker({
	  ...
	  checkin.hide();
	  $('#dp'+counter+'-p')[0].focus();
	}).data('datepicker');

	var checkout = $('#dp'+counter+'-p').datepicker({
	  onRender: function(date) {
...
$(document).ready(function(){
	$('#bulk_update_form1').submit(function(event){
		event.preventDefault();
		$.post(base_url+'/inventory/bulk_update/update',$(this).serializeArray());
		$('#bulk_update_form1').trigger('reset');
	});
